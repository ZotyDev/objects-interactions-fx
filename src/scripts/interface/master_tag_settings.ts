...
import { ObjectsInteractionsFXData as OIFD } from '../data/oif_data';
import { MasterTagConfiguration } from "./master_tag_configuration";
...
export class MasterTagsSettings extends FormApplication {
  ...
  // ? Only GMs should have the default tag packs always loaded, there is no
  // ? reason for other users having access to it.
  ////////////////////////////////////////////////////////////////////////////
  static async loadDefaultTagPacks() {
    const DefaultTagPacks =
    [
      'empty',
      'fantasy_jb2a_complete',
      'fantasy_jb2a_free',
      'fantasy_no_animations'
    ];

    for (const path of DefaultTagPacks)
    {

      await MasterTagsSettings.LoadTagPackFromFile(`modules/${OIF.ID}/module/${path}.json`);
    }
  }
