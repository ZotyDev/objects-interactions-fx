...
import { ItemDropper} from "../library/item_dropper";
import { InventoryManipulator } from "../library/inventory_manipulator";
...
export class ItemAnimator
{
  ...
  */
  static async MeleeWeaponSingleAttack(options)
  {
    ...
    C.D.warn(itemInformation);

    // Check if item can be thrown at the current distance
    if (itemInformation.properties.thrown && options.distance >= itemInformation.range.melee + options.gridUnitSize) {
      ...
      if (options.distance <= itemInformation.range.max) {
        ...
        const throwProjectile = new Projectile({
          name: options.name,
          author: options.token,
          tags: options.tags,
          animation: {
            fire: {
              source: options.throwAnimation.source,
              delay: options.throwAnimation.delay,
            }
          },
          item: {
            source: options.item,
            breakChance: GeneralSettings.Get(OIF.SETTINGS.GENERAL.DEFAULT_THROWABLE_DESTRUCTION_CHANCE),
            dropOnMiss: GeneralSettings.Get(OIF.SETTINGS.GENERAL.CREATE_ITEM_PILES_ON_MISS),
            dropOnHit: GeneralSettings.Get(OIF.SETTINGS.GENERAL.CREATE_ITEM_PILES_ON_HIT),
            addToTarget: GeneralSettings.Get(OIF.SETTINGS.GENERAL.ADD_THROWABLE_TO_TARGET_INVENTORY),
            remove: GeneralSettings.Get(OIF.SETTINGS.GENERAL.REMOVE_THROWABLE_ITEM),
          },
          targets: [
            {
              target: options.target,
              miss: options.miss,
              powerful: options.throwAnimation.powerful,
            }
          ]
        });
    ...
    else
    {
      ...
      if (options.distance < itemInformation.range.melee + options.gridUnitSize)
      {
        ////////////////////////////////////////////////////////////
        // Melee Attack
        ////////////////////////////////////////////////////////////
